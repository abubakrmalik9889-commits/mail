...
export default function Templates() {
  ...
  async function handleAIGenerate() {
    if (!aiPrompt.trim()) return showToast('Enter a prompt first');
    setGenerating(true);
    try {
      const result = await api.generateAITemplate(aiPrompt);
      setEditing(p => ({ ...p, subject: result.subject, body: result.body }));
      showToast('AI Generated content!');
      setAiPrompt('');
    } catch (e) {
      showToast(e.message || 'AI generation failed');
    } finally {
      setGenerating(false);
    }
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '210px 1fr 1fr', gap: 16, height: 'calc(100vh - 120px)', minHeight: 500 }}>
      ...
      <Card style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }} padding="0">
        ...
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 13, flex: 1, overflowY: 'auto' }}>
          <div style={{ background: 'var(--brand-light)', border: '1px solid var(--brand-border)', borderRadius: 'var(--radius-sm)', padding: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              <span>🪄 AI Magic Quill</span>
              {generating && <Spinner size={12} />}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                type="text"
                placeholder="Write a job invite for developers..."
                value={aiPrompt}
                onChange={e => setAiPrompt(e.target.value)}
                style={{ flex: 1, fontSize: 12, padding: '8px 12px', borderRadius: 6, border: '1px solid var(--border)', outline: 'none', background: 'white' }}
              />
              <Button size="sm" onClick={handleAIGenerate} disabled={generating}>Generate</Button>
            </div>
          </div>
