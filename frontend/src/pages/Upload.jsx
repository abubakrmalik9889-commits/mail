...
export default function Upload() {
  ...
  async function launch() {
    ...
    const smtpConfig = JSON.parse(localStorage.getItem('mailflow_smtp') || '{}');
    if (!smtpConfig.user || !smtpConfig.pass) { showToast('⚙️ Please configure SMTP settings first'); return navigate('/settings'); }
    setLaunching(true);
    try {
      await api.createCampaign({ name: campaignName, templateId, leads, smtpConfig });
      setLaunched(true);
